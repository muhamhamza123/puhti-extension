...
class PuhtiWidget extends Widget {
    ...
    _build() {
        ...
        const panels = [];
        tabs.forEach((name, i) => {
            const t = el('button', 'flex:1;padding:8px 2px;border:none;background:none;cursor:pointer;font-size:11px;' +
    ...
    _buildSubmit(p) {
        ...
        this._timeRange = el('input', 'flex:1;');
        this._timeRange.type = 'range';
        this._timeRange.min = '1';
        this._timeRange.max = '72';
        this._timeRange.value = '2';
        this._timeLabel = el('span', 'font-size:12px;width:36px;text-align:right;', '2 h');
    ...
    async _requestSimpleContainer() {
        ...
        fd.append('username', this._username);
        const emailVal = (_b = (_a = this._emailInput) === null || _a === void 0 ? void 0 : _a.value) === null || _b === void 0 ? void 0 : _b.trim();
        if (emailVal) {
            fd.append('email', emailVal);
        }
        try {
    ...
    async _requestContainer() {
        ...
        fd.append('username', this._username);
        const emailVal2 = (_b = (_a = this._emailInput) === null || _a === void 0 ? void 0 : _a.value) === null || _b === void 0 ? void 0 : _b.trim();
        if (emailVal2) {
            fd.append('email', emailVal2);
        }
        try {
    ...
    async _loadContainerRequests() {
        ...
        try {
            ...
            const STATUS_COLOR = { pending: '#f59e0b', merged: '#3b82f6', building: '#3b82f6', ready: '#10b981', closed: '#ef4444' };
            reqs.forEach(r => {
    ...
    // ── Data tab ────────────────────────────────────────────────────────────────
    _buildData(p) {
        const pathBanner = el('div', 'font-size:11px;background:var(--jp-layout-color2);border-radius:5px;padding:7px 10px;line-height:1.6;');
        ...
        this._dataFileInput = el('input', 'display:none;');
        ...
        this._dataFileLabel = el('span', 'font-size:11px;color:var(--jp-ui-font-color2);margin-left:8px;', 'No files chosen');
        ...
        this._dataUploadStatus = el('div', 'font-size:12px;min-height:18px;');
        ...
        this._dataFileList = el('div', 'display:flex;flex-direction:column;gap:4px;');
        p.appendChild(this._dataFileList);
    }
    async _uploadData(uploadBtn) {
        const files = Array.from(this._dataFileInput.files || []);
        if (!files.length) {
        ...
        try {
            ...
            const msg = `✓ Uploaded: ${result.uploaded.join(', ')}` +
    ...
    _makePathPanel(fullPath) {
        const panel = el('div', 'display:none;padding:4px 8px 4px 12px;background:var(--jp-layout-color0);' +
    ...
    async _loadDataFiles() {
        if (!this._username || !this._dataFileList) {
            return;
        }
        ...
        try {
            ...
            groups.forEach((folderFiles, folder) => {
                ...
                const folderWrap = el('div', 'display:flex;flex-direction:column;gap:2px;');
                // Folder header row
                const folderRow = el('div', 'display:flex;align-items:center;gap:6px;font-size:11px;padding:5px 8px;' +
                ...
                const children = el('div', 'display:none;flex-direction:column;gap:2px;padding-left:12px;');
                folderRow.onclick = () => {
    ...
    async _deleteDataFile(filename, wrap) {
