...
class PuhtiWidget extends Widget {
    ...
    // ── Data tab ────────────────────────────────────────────────────────────────
    _buildData(p) {
        p.appendChild(el('div', 'font-size:11px;color:var(--jp-ui-font-color2);line-height:1.5;', 'Upload files to your persistent data directory on Puhti. Access them in your code via os.environ["USERDATA_PATH"].'));
    ...
    async _uploadData(uploadBtn) {
        ...
        try {
            ...
            this._setStatus(this._dataUploadStatus, `✓ Uploaded: ${result.uploaded.join(', ')}`, '#10b981');
    ...
    async _loadDataFiles() {
        ...
        try {
            ...
            files.forEach(f => {
                const row = el('div', 'display:flex;align-items:center;gap:6px;font-size:11px;' +
    ...
    async _deleteDataFile(filename, row) {
        if (!confirm(`Delete "${filename}" from Puhti?`)) {
            return;
        }
        try {
